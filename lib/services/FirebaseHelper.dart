...
import 'package:custom_food/model/AttributesModel.dart';
import 'package:custom_food/model/BannerModel.dart';
import 'package:custom_food/model/BlockUserModel.dart';
import 'package:custom_food/model/BookTableModel.dart';
import 'package:custom_food/model/ChannelParticipation.dart';
import 'package:custom_food/model/ChatVideoContainer.dart';
import 'package:custom_food/model/CodModel.dart';
import 'package:custom_food/model/CurrencyModel.dart';
import 'package:custom_food/model/DeliveryChargeModel.dart';
import 'package:custom_food/model/FavouriteItemModel.dart';
import 'package:custom_food/model/FavouriteModel.dart';
import 'package:custom_food/model/FlutterWaveSettingDataModel.dart';
import 'package:custom_food/model/MercadoPagoSettingsModel.dart';
import 'package:custom_food/model/OrderModel.dart';
import 'package:custom_food/model/PayFastSettingData.dart';
import 'package:custom_food/model/PayStackSettingsModel.dart';
import 'package:custom_food/model/ProductModel.dart';
import 'package:custom_food/model/Ratingmodel.dart';
import 'package:custom_food/model/ReviewAttributeModel.dart';
...
import 'package:custom_food/model/paytmSettingData.dart';
import 'package:custom_food/model/razorpayKeyModel.dart';
import 'package:custom_food/model/story_model.dart';
import 'package:custom_food/model/stripeKey.dart';
import 'package:custom_food/model/stripeSettingData.dart';
import 'package:custom_food/model/topupTranHistory.dart';
...
class FireStoreUtils {
  ...
  static Future<User?> getCurrentUser(String uid) async {
    print("OBTENIENDO USUARIO $uid con $USERS");
    DocumentSnapshot<Map<String, dynamic>> userDocument =
        await firestore.collection(USERS.toString()).doc(uid.toString()).get();
    if (userDocument.data() != null && userDocument.exists) {
  ...
  static Future<void> updateAddressList(NewAddressModel userAddress) async {
    return await firestore
        .collection(USERS)
        .doc(MyAppState.currentUser!.userID)
      ...
      await Future.forEach(onData.docs,
        ...
        try {
          ...
          if (!orders
            ...
            orders[orders.indexWhere((e) => e.id == orderModel.id)] =
                orderModel;
          }
        } catch (e, s) {
  ...
  Future<List<VendorModel>> getRestaurants() async {
    List<VendorModel> vendors = [];
    ...
    vendors = List.from(query.docs).map((element) {
  ...
  Future<List> getcountryList() async {
    var collection = FirebaseFirestore.instance.collection(Setting);
    var docSnapshot = await collection.doc('Countries').get();
    Map<String, dynamic>? data = docSnapshot.data();
    var value = data?['list'];
    countries = value;
    return countries;
  ...
  Future<List<OfferModel>> getCoupons(String vendorID) async {
    List<OfferModel> coupon = [];

    QuerySnapshot<Map<String, dynamic>> couponsQuery = await firestore
        .collection(COUPON)
        .where("isEnabled", isEqualTo: true)
        .where("restaurant_id", arrayContains: vendorID)
        .get();
    couponsQuery.docs
        .forEach((QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
  ...
  Future<List<TopSellingModel>> getTopSelling() async {
    List<TopSellingModel> top = [];

    QuerySnapshot<Map<String, dynamic>> topsellsQuery =
        await firestore.collection(TOP_SELLING).get();
    topsellsQuery.docs
        .forEach((QueryDocumentSnapshot<Map<String, dynamic>> document) {
      try {
        top.add(TopSellingModel.fromJson(document.data()));
      } catch (e) {
        print('FireStoreUtils.getTopSelling Parse error $e');
      }
    });
    return top;
  }
  ...
  static loginWithGoogle() async {
    ...
    return await handleGoogleLogin(googleUser, googleAuth);
  }

  static handleGoogleLogin(GoogleSignInAccount userData,
    ...
    List<String> fullName = (userData.displayName as String).split(' ');
    String firstName = '';
    String lastName = '';
    ...
    if (user != null && user.role == USER_ROLE_CUSTOMER) {
      user.profilePictureURL = userData.photoUrl ?? "";
